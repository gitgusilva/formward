...
export const getForm = (el: HTMLInputElement) => {
  ...
  return !isNullOrUndefined(el.parentNode) ? getForm(el.parentNode as HTMLInputElement) : null;
};

export const getPath = (path: string, target: object | null | undefined, def: any = undefined) => {
 ...
 */
export const parseRule = (rule: string): { name: string; params: string[] } => {
  let params: string[] = [];
...
/**
 * Debounces a function. If wait is 0, returns the function unchanged (immediate).
 * @param {Function} fn - Function to debounce.
 * @param {number} [wait=0] - Delay in ms.
 * @param {{ cancelled: boolean }} [token] - Optional token to cancel pending invocation.
 * @returns {Function}
 */
export const debounce = (fn: (...args: any[]) => any, wait: number = 0, token: { cancelled: boolean } = { cancelled: false }) => {
  if (wait === 0) return fn;
 ...
 */
export const appendRule = (rule: string, rules: string | Record<string, boolean | any[]>) => {
  if (!rules) {
...
/**
 * Normalizes rules from string ('required|email|min:3') or object to a rules map.
 * @param {string|Object} rules - Pipe-separated string or { ruleName: true|params }.
 * @returns {Object} Map of rule name to params array.
 */
export const normalizeRules = (rules: string | Record<string, boolean | any[]>) => {
  ...
  if (isObject(rules)) {
    return Object.keys(rules).reduce<Record<string, any>>((prev, curr) => {
      let params: any[];
      if (rules[curr] === true) {
        params = [];
  ...
  return rules.split('|').reduce<Record<string, any>>((prev, rule) => {
    const parsedRule = parseRule(rule);
    if (!parsedRule.name) return prev;
...
export const warn = (message: string) => {
  console.warn(`[formward] ${message}`); // eslint-disable-line
};

/**
 * Creates a branded error object.
 */
export const createError = (message: string): Error => new Error(`[formward] ${message}`);
 ...
 */
export const toggleClass = (el: HTMLElement | null | undefined, className: string | string[] | null | undefined, status: boolean) => {
 ...
 */
export const ensureArray = (arrayLike: any): any[] => {
...
export const isBuiltInComponent = (vnodeOrVm: Object): boolean => {
  if (!vnodeOrVm) {
    return false;
  }
...
export const fillRulesFromElement = (el: HTMLInputElement, rules: string | Record<string, boolean | any[]>) => {
  if (el.required) {
