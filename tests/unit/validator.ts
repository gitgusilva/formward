...
const Validator = Formward.Validator;
Formward.install(Vue);
...
test('rules can be skipped on initial validation', async () => {
  const v = new Validator();
  const validateSkip = vi.fn(() => true);
  const validateUnskipped = vi.fn(() => true);
