...
test('debounces the provided function', () => {
...
test('calls functions immediatly if time is 0', () => {
  const [value, argument] = ['someval', 'somearg'];
  const func = utils.debounce((val, arg) => {
    expect(val).toBe(value);
    expect(arg).toBe(argument);
...
test('warns with branded message', () => {
  ...
  expect(warnSpy).toHaveBeenCalledWith('[formward] Something is not right');
  warnSpy.mockRestore();
});
...
test('checks if formward is available globally', () => {
  expect(i18Utils.isDefinedGlobally()).toBe(false);
  (global as any).Formward = { myprop: true };
