...
Vue.use(Formward, { inject: false });
Vue.component('ValidationProvider', Formward.ValidationProvider);
Vue.component('ValidationObserver', Formward.ValidationObserver);
...
describe('Validation Provider Component', () => {
  ...
  test('removes the provider reference at destroy', () => {
    ...
    const providersMap = wrapper.vm.$_formwardObserver.refs;
  ...
  test('creates HOCs from other components', async () => {
    const InputWithValidation = Formward.withValidation(InputWithoutValidation);
  ...
  test('renders slots', async () => {
    const WithValidation = Formward.withValidation(SelectWithoutValidation);
    const wrapper = mount({
      data: () => ({ value: '' }),
      template: `
  ...
  test('avoids race conditions between successive validations', async () => {
    ...
    Formward.Validator.extend('longRunning', {
