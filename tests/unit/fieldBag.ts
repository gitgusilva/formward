...
test('adds field items to the collection', () => {
  ...
  expect(() => {
    bag.push(field);
  }).toThrow(`[formward] Field with id ${field.id} is already added.`);

  expect(() => {
    field.id = null;
    bag.push(field);
  }).toThrow('[formward] Field id must be defined');

  // test type check.
  expect(() => {
    bag.push('generic');
  }).toThrow('[formward] FieldBag only accepts instances of Field');
});
